    ...
    function MSECalendar() {
      ...
      let fcRecurSources = {
        AUT24Events: function(info, successCallback, failureCallback) {
          http.get('http://localhost:4000/AUT24').subscribe((data: any) => {
            let AUT24Events: any[] = [];
        ...
        SPR24ComWeek: function(info, successCallback, failureCallback){
          http.get('http://localhost:5000/SPR24ComWeek').subscribe((data: any) => {
            ...
            data.forEach((event: any) => {
              SPR24ComWeek.push({
                ...
                extendedProps: {
                  ...
                  duration: event.startTime + ' - ' + event.endTime,
                  room: event.comRoom,
                  buildingRoom: event.comVenue + '  ' + event.comRoom,
                  modeType: event.modeType,
                }
              })
            }),
        ...
        SPR24Events: function(info, successCallback, failureCallback) {
          http.get('http://localhost:3000/SPR24').subscribe((data: any) => {
            let SPR24Events: any[] = [];
            data.forEach((event: any) => {
              SPR24Events.push({
                title: event.id,
